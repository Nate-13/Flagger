// Test the main-world component probe end to end. jsdom has a single JS world,
// so the injected probe <script> runs in the same window as the overlay — which
// still exercises the full postMessage round-trip (probe request → fiber walk →
// result → flag.component filled → re-persist). We fake a React fiber on the
// flagged element to stand in for a dev build.
